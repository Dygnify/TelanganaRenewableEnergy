...
const ConfigureWeightage = ({ tableData, setFinalScore }) => {
  ...
  const handleSliderChange = (id, newValue) => {
    ...
    for (let i = 0; i < len; i++) {
      ...
      tableData[i].score = (tableData[i].value * tableData[i].weightage) / 100;
      currentTotalScore += tableData[i].score;
    }
          ...
          <table className="table-auto w-full">
            <thead className="text-xs font-semibold uppercase text-gray-400 bg-gray-50">
              <tr>
                ...
                <th className="p-2 w-4/12">
                  <div className="font-semibold center">Weightage</div>
                </th>
              </tr>
            </thead>
              ...
              {tableData.map((data, i) => (
                <tr key={i}>
                  <td className="p-2 w-4/12">
                    <div className=" text-gray-800">{data.parameter}</div>
                  </td>
                  <td className="p-2 w-4/12">
                    <div className="">
                      <RangeSlider
